...
class BackendController extends BackendbaseController {
    ...
    public function tinysave(){
        ...
        $settings->autoload = 1;
        if($settings->save()){
            return \Redirect::to('backend/tinymce')->withMessage($this->notifyView(t('messages.settings_saved'),'success'));
        }
        return \Redirect::to('backend/tinymce')->withMessage($this->notifyView(t('messages.error_occured'),'danger'));
    }
